...
int main(void) {
    ...
    for (map[newh] = -1; !die && system("cls") == 0; ) {
        if (eat = map[newh] == -1, map[newh] = map[head] + 1, head = newh, map[head] == 19 * 15 + 1) break; // eat food and check win
        for (i = 0, n = rand() % (19 * 15 + 1 - map[head]); eat && i < 19 * 15; i++) { if (!map[i] && --n <= 0) { map[i] = -1; break; } } // new food
    ...
    printf("\nyou %s !\n", map[head] == 19 * 15 ? "win" : "lost"); (void)_getch(); return 0;
}